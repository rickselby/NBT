...
class Service
{
    ...
    public function writeType( $fp, $tagType, $value )
    {
        switch ( $tagType ) {
            ...
            case self::TAG_SHORT: // Signed short (16 bit, big endian)
                if ( $value < 0 )
                    $value += pow( 2, 16 ); // Convert signed short to unsigned short
                return is_int( fwrite( $fp, pack( "n", $value ) ) );
            case self::TAG_INT: // Signed integer (32 bit, big endian)
                if ( $value < 0 )
                    $value += pow( 2, 32 ); // Convert signed int to unsigned int
                return is_int( fwrite( $fp, pack( "N", $value ) ) );
            case self::TAG_LONG: // Signed long (64 bit, big endian)
                if ( PHP_INT_SIZE >= 8 ) {
            ...
            case self::TAG_LIST: // List
                if ( $this->verbose )
                    trigger_error( "Writing list of " . count( $value[ "value" ] ) . " tags of type {$value[ "type" ]}.", E_USER_NOTICE );
